...
import static cloud.xcan.sdf.spec.experimental.BizConstant.CTRL_SERVICE;
import static cloud.xcan.sdf.spec.experimental.BizConstant.CTRL_SERVICE_PRIVATIZATION;
import static cloud.xcan.sdf.spec.experimental.BizConstant.GM_SERVICE;
import static cloud.xcan.sdf.spec.experimental.BizConstant.GM_SERVICE_PRIVATIZATION;
import static cloud.xcan.sdf.spec.experimental.BizConstant.TESTER_SERVICE;
import static cloud.xcan.sdf.spec.experimental.BizConstant.TESTER_SERVICE_PRIVATIZATION;
...
public class EnumStoreInMemory implements EnumStore {
  ...
  @Override
  public void init() {
    Reflections reflections = new Reflections("cloud.xcan.sdf", "cloud.xcan.angus",
    ...
    if (isNotEmpty(classes)) {
      for (Class<? extends Value> clz : classes) {
        if (clz.isEnum()) {
          ...
          boolean isSdfEnum = clz.getName().startsWith("cloud.xcan.sdf.api.enums")
              || clz.getName().startsWith("cloud.xcan.sdf.spec")
          ...
          String key = clz.getName().replaceFirst("cloud.xcan.sdf", "xcm");
