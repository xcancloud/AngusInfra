...
import static cloud.xcan.sdf.api.ApiConstant.EVENT_PATTERN;
import static cloud.xcan.sdf.api.ApiConstant.LOG_PATTERN;
import static cloud.xcan.sdf.api.ApiConstant.Service.EVENT_SERVICE;
import static cloud.xcan.sdf.api.ApiConstant.Service.LOGGER_SERVICE;
import static cloud.xcan.sdf.core.utils.PrincipalContextUtils.getApplicationInfo;
import static cloud.xcan.sdf.core.utils.ServletUtils.getRequestId;
import static cloud.xcan.sdf.spec.SpecConstant.DEFAULT_ENCODING;
import static cloud.xcan.sdf.spec.http.ContentTypes.isBinaryContent;
import static cloud.xcan.sdf.spec.http.ContentTypes.isFormData;
import static cloud.xcan.sdf.spec.principal.PrincipalContext.getApiType;
import static cloud.xcan.sdf.spec.principal.PrincipalContext.getClientSource;
...
import cloud.xcan.sdf.core.app.AppBeanReady;
...
import cloud.xcan.sdf.core.log.ApiLogProperties.SystemRequest;
import cloud.xcan.sdf.core.log.ApiLogProperties.UserRequest;
...
public class ApiLogFilter extends OncePerRequestFilter implements AppBeanReady {
    ...
    for (Enumeration<?> names = wrapper.getHeaderNames(); names.hasMoreElements(); ) {
      ...
      for (Enumeration<?> headerValues = wrapper.getHeaders(headerName);
        ...
        if (Header.AUTHORIZATION.equals(headerName)) {
          ...
          // @see cloud.xcan.sdf.api.commonlink.AASConstant.DEFAULT_TOKEN_SALT_LENGTH = 32;
  ...
  private ApiLog buildApiLogRequest(String requestId, HttpServletRequest request, int bodyLength,
    ...
    return ApiLog.newBuilder().requestId(requestId)
        ...
        .userId(principal.getUserId()).fullname(principal.getUserFullname())
