...
4. **Locale 回退** — 请求标签 → 语言族 → 配置默认（`en`）→ fallback / 原始 key。
5. **分层国际化** — 枚举走 classpath；配置文案走本模块；实体内容另建模型。

**默认语言（单一来源）：** `BizConstant.DEFAULT_LANGUAGE`（`en`）。
`SupportedLanguage.defaultLanguage()`、`SpecConstant.DEFAULT_LOCALE`、
`angus.i18n.message.default-locale` 均由此派生，禁止另写平行默认值。
...
| `default-locale` | `BizConstant.DEFAULT_LANGUAGE`（`en`） | 应用默认语言（`SupportedLanguage` 枚举名）；仅必要时覆盖 |
...
中文请求（`zh` / `zh-CN` / `zh-TW` / `zh-HK` 等均规范为 `zh_CN`）候选顺序：
...
**请求语言策略**（`SupportedLanguage`）：

| 输入 | 解析结果 |
|------|----------|
| `en` / `en-US` / `en_US` / `en-GB` | `en` |
| `zh` / `zh-CN` / `zh_CN` / `zh-TW` / `zh-HK` | `zh_CN`（目前仅简体） |
| 未知 / 非法 | 系统默认（`en`） |

入库的 `language` 建议与枚举名完全一致：`en`、`zh_CN`。
