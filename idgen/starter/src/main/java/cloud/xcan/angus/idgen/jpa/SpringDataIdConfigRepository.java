...
public interface SpringDataIdConfigRepository extends JpaRepository<IdConfig, String> {
  ...
  @Modifying
  @Query("update angus_id_config t set t.maxId = t.maxId + ?1 where t.bizKey = ?2 and t.tenantId = ?3")
  int incrementByBizKeyAndTenantId(Long step, String bizKey, Long tenantId);

  @Lock(value = LockModeType.PESSIMISTIC_WRITE)
  @Query("select t.maxId from angus_id_config t where t.bizKey = ?1 and t.tenantId = ?2")
