 ...
 * collects all results before calling {@link #reduce} on a single node.
 * Implementations must also provide an {@link #execute} method, but the
 * framework never calls it directly — it is provided for API symmetry only
 * and should return {@code null}.
 */
