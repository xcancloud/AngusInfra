...
public class JobProperties {
  ...
  /**
   * Interval in milliseconds between successive scans for due jobs. Lower values reduce scheduling
   * latency but increase DB read load.
   */
  private long scanIntervalMs = 1_000;

  /**
   * Maximum number of seconds that a distributed lock is held. Should be set comfortably above the
   * expected worst-case job duration.
   */
  ...
  /**
   * Maximum number of due jobs the scheduler will load per scan cycle. Prevents a sudden spike of
   * overdue jobs from exhausting the executor pool or heap. Tune upward only when the job-executor
   * pool has enough capacity.
   */
