...
@NoRepositoryBean
public interface ExecSampleScoreRepo extends BaseRepository<ExecSampleScore, Long> {

  @Sharding
  @Override
  Page<ExecSampleScore> findAll(Specification<ExecSampleScore> spec, Pageable pageable);
  ...
  @Query(value =
      ...
      nativeQuery = true)
  List<ExecSampleScore> findAllByTenantIdAndExecIdAndTimestampAndNameInAndNodeIdIn(long tenantId,
  ...
  @Query(value =
      "SELECT ep.* FROM exec_sample ep WHERE ep.tenant_id = ?1 AND ep.exec_id = ?2 "
          ...
          + "ORDER BY ep.timestamp ASC",
      nativeQuery = true)
  List<ExecSampleScore> findAllByTenantIdAndExecIdAndTimestampAndNameNotAndNodeIdIn(long tenantId,
      long execId, String startTime, String endTime, String name, Collection<Long> nodeIds);

  @Override
  @Sharding
  List<ExecSampleScore> findAllByFilters(Set<SearchCriteria> filters);
}
