 ...
 * <p>
 * Implementations expose access to the Spring {@link ApplicationContext}, configuration
 * and data directory for the plugin, simple logging utilities, and a service registry
 * where plugins can publish or look up shared services.
 */
public interface PluginContext {

    /**
     * Return the Spring ApplicationContext used by the host application. Plugins may
     * use this to look up beans or publish their own beans (if appropriate).
     *
    ...
    /**
     * Return the plugin-specific configuration map. This typically contains values from
     * the plugin descriptor merged with host-provided overrides.
     *
    ...
    /**
     * Directory on disk where the plugin may store persistent or ephemeral data. The
     * host is responsible for creating this directory before the plugin writes to it.
     *
    ...
    /**
     * Log a message at the given level using the host's logging infrastructure. Level
     * is typically one of INFO/WARN/ERROR/DEBUG but is not restricted by the API.
     *
    ...
    /**
     * Register a service object under the given name so other plugins or the host can
     * look it up. Implementations should document any thread-safety or lifecycle semantics.
     *
