...
public class DynamicRestEndpointManager {
    ...
    public boolean registerPlugin(String pluginId, RestfulPlugin plugin, PluginContext context) {
        try {
            ...
            if (controllerClasses == null || controllerClasses.isEmpty()) return true;
            List<RequestMappingInfo> mappingInfos = new ArrayList<>();
            List<Object> controllers = new ArrayList<>();
            String apiPrefix = plugin.getApiPrefix();
            if (apiPrefix == null || apiPrefix.isEmpty()) apiPrefix = "/api/plugins/" + pluginId;
            for (Class<?> controllerClass : controllerClasses) {
                try {
                    Object controller = createControllerInstance(controllerClass, context);
                    controllers.add(controller);
                    List<RequestMappingInfo> controllerMappings = registerControllerMethods(controller, controllerClass, apiPrefix);
            ...
            log.info("Successfully registered {} REST endpoints for plugin: {}", mappingInfos.size(), pluginId);
            return true;
        } catch (Exception e) {
    ...
    private Object createControllerInstance(Class<?> controllerClass, PluginContext context) throws Exception {
        Object controller = controllerClass.getDeclaredConstructor().newInstance();
        if (controller instanceof PluginController) ((PluginController) controller).setPluginContext(context);
        applicationContext.getAutowireCapableBeanFactory().autowireBean(controller);
        return controller;
    }

    private List<RequestMappingInfo> registerControllerMethods(Object controller, Class<?> controllerClass, String apiPrefix) {
        List<RequestMappingInfo> mappingInfos = new ArrayList<>();
    ...
    private RequestMappingInfo createMappingInfo(Method method, String[] classLevelPaths, String apiPrefix) {
    ...
    private String combinePaths(String... paths) {
        ...
        for (String path : paths) {
            ...
            if (!path.startsWith("/")) result.append("/");
            result.append(path);
        }
    ...
    public List<EndpointInfo> getPluginEndpoints(String pluginId) {
        ...
        if (mappingInfos == null) return Collections.emptyList();
        List<EndpointInfo> out = new ArrayList<>();
        for (RequestMappingInfo info : mappingInfos) out.add(toEndpointInfo(info));
        return out;
    }

    public Map<String, List<EndpointInfo>> getAllPluginEndpoints() {
        Map<String, List<EndpointInfo>> result = new HashMap<>();
        pluginEndpoints.forEach((pluginId, mappings) -> result.put(pluginId, getPluginEndpoints(pluginId)));
        return result;
    }
    ...
    private EndpointInfo toEndpointInfo(RequestMappingInfo mappingInfo) {
        ...
        return EndpointInfo.builder().path(patterns.isEmpty() ? "" : patterns.iterator().next()).methods(methods.stream().map(Enum::name).toList()).build();
    }
