...
/**
 * A triple of simple moving average rates (one, five and fifteen minutes rates) as needed by
 * {@link Meter}.
 * <p>
 * The averages are unweighted, i.e. they include strictly only the events in the sliding time
 * window, every event having the same weight. Unlike the the more widely used
 * {@link ExponentialMovingAverages} implementation, with this class the moving average rate drops
 * immediately to zero if the last marked event is older than the time window.
 * <p>
 * A {@link Meter} with {@link SlidingTimeWindowMovingAverages} works similarly to a
 * {@link Histogram} with an {@link SlidingTimeWindowArrayReservoir}, but as a Meter needs to keep
 * track only of the count of events (not the events itself), the memory overhead is much smaller.
...
public class SlidingTimeWindowMovingAverages implements MovingAverages {
  ...
  private static final Duration TIME_WINDOW_DURATION = Duration
      .ofMinutes(TIME_WINDOW_DURATION_MINUTES);
   ...
   */
  private final ArrayList<LongAdder> buckets;
  /**
  ...
  private void cleanOldBuckets(Instant currentTick) {
    int newOldestIndex;
    Instant oldestStillNeededTime = currentTick.minus(TIME_WINDOW_DURATION)
        .plusNanos(TICK_INTERVAL);
