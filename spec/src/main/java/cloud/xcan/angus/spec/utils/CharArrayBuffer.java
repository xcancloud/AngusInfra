...
import static cloud.xcan.angus.spec.utils.StringUtils.isWhitespace;
...
public final class CharArrayBuffer implements CharSequence, Serializable {
  ...
  /**
   * Appends {@code len} chars to this buffer from the given source array starting at index
   * {@code off}. The capacity of the buffer is increased, if necessary, to accommodate all
   * {@code len} chars.
   *
  ...
  /**
   * Appends {@code len} chars to this buffer from the given source buffer starting at index
   * {@code off}. The capacity of the destination buffer is increased, if necessary, to accommodate
   * all {@code len} chars.
   *
   ...
   * capacity of the destination buffer is increased, if necessary, to accommodate all
   * {@link #length()} chars.
   *
   * @param b the source buffer to be appended.
   */
  ...
  /**
   * Appends {@code len} bytes to this buffer from the given source array starting at index
   * {@code off}. The capacity of the buffer is increased, if necessary, to accommodate all
   * {@code len} bytes.
   * <p>
  ...
  /**
   * Returns {@code true} if this buffer is empty, that is, its {@link #length()} is equal to
   * {@code 0}.
   *
   ...
   * <p>
   * There is no restriction on the value of {@code beginIndex} and {@code endIndex}. If
   * {@code beginIndex} is negative, it has the same effect as if it were zero. If {@code endIndex}
   * is greater than {@link #length()}, it has the same effect as if it were {@link #length()}. If
   * the {@code beginIndex} is greater than the {@code endIndex}, {@code -1} is returned.
   *
   ...
   * @throws StringIndexOutOfBoundsException if the {@code beginIndex} is negative, or
