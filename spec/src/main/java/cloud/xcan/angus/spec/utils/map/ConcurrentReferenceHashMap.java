package cloud.xcan.angus.spec.utils.map;
 ...
 * {@code Collections.synchronizedMap(new WeakHashMap<K, Reference<V>>())} in order to multitenancy
 ...
 * placed into the map will be subsequently available. The garbage collector may discard references
...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V> implements
  ...
  public ConcurrentReferenceHashMap(
    ...
    assertTrue(initialCapacity >= 0, "Initial capacity must not be negative");
    assertTrue(loadFactor > 0f, "Load factor must be positive");
    assertTrue(concurrencyLevel > 0, "Concurrency level must be positive");
    assertNotNull(referenceType, "Reference type must not be null");
  ...
  private V put(@Nullable final K key, @Nullable final V value, final boolean overwriteExisting) {
    return doTask(key, new Task<V>(TaskOption.RESTRUCTURE_BEFORE, TaskOption.RESIZE) {
      ...
      protected V execute(@Nullable Reference<K, V> ref, @Nullable Entry<K, V> entry,
        ...
        assertState(entries != null, "No entries segment");
        entries.add(value);
        return null;
      }
    });
  ...
  /**
   * Allows a task access to {@link class -> Segment} entries.
   */
  ...
  private class EntryIterator implements Iterator<Map.Entry<K, V>> {
    ...
    @Override
    public void remove() {
      assertState(this.last != null, "No element to remove");
   ...
   */
  private static final class SoftEntryReference<K, V> extends SoftReference<Entry<K, V>> implements
      Reference<K, V> {
   ...
   */
  private static final class WeakEntryReference<K, V> extends WeakReference<Entry<K, V>> implements
      Reference<K, V> {
