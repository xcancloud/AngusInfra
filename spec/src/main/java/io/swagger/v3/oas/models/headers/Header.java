...
public class Header {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(description, required, deprecated, style, explode, schema, examples,
        example, content, extensions, $ref);
  }
  ...
  public static String format$ref(String $ref) {
    if ($ref != null && ($ref.indexOf('.') == -1 && $ref.indexOf('/') == -1)) {
      return Components.COMPONENTS_HEADERS_REF + $ref;
    }
    return $ref;
  }
